...
function generateRandomFlagKey(template: FeatureFlagTemplate): string {
  const suffix = suffixes[Math.floor(Math.random() * suffixes.length)];
  const randomNum = Math.floor(Math.random() * 9999) + 1;
  return `${template.prefix}_${suffix}_${randomNum}`;
}

function generateRandomFlagName(template: FeatureFlagTemplate): string {
  const description = template.descriptions[Math.floor(Math.random() * template.descriptions.length)];
  const suffix = suffixes[Math.floor(Math.random() * suffixes.length)];
  return `${description} ${suffix}`;
}

function generateRandomDescription(template: FeatureFlagTemplate): string {
  const descriptions = [
...
async function generateChangeRequests(users: any[]) {
...
export async function generateFeatureFlags() {
  try {
